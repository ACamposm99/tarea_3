...
def ver_ventas():
    ...
    if conn:
        try:
            ...
            if ventas:
                st.subheader(f"Total de ventas: {len(ventas)}")
                total_ventas = sum(venta['total'] for venta in ventas)
                col1, col2 = st.columns(2)
                with col1: st.metric("💰 Total Recaudado", f"${total_ventas:,.2f}")
                with col2: st.metric("📈 Número de Ventas", len(ventas))
                
                for venta in ventas:
                    with st.expander(f"🛒 Venta #{venta['id']} - ${venta['total']:.2f}"):
                        col1, col2 = st.columns(2)
                        with col1:
    ...
    if conn:
        try:
            ...
            if not clientes:
                ...
                return
            if not productos:
                st.warning("⚠️ No hay productos con stock")
                return
